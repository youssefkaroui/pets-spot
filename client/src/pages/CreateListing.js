...
const CreateListing = () => {
  ...
  const handlePetCreate = async (e) => {
    ...
    const petImage = `https://${process.env.REACT_APP_S3_BUCKET}.s3.amazonaws.com/${imageFile.name}`
    try {
      S3Upload(imageFile)
  ...
  const handleFileUpload = async (e) => {
    ...
    //Allow user input image to be read as dataurl
    newImage.readAsDataURL(file)
    //assign image as new src
